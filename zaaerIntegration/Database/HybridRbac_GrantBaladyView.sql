/*
  Grant integrations.balady.view to roles that can view platform integrations.
  Run after HybridRbac_SeedPermissions.sql
*/

SET NOCOUNT ON;

DECLARE @newCode NVARCHAR(150) = N'integrations.balady.view';
DECLARE @legacyView NVARCHAR(150) = N'integrations.view';
...
WHERE p.permission_code = @legacyView
  AND rp.granted = 1
  AND NOT EXISTS (
      SELECT 1
...
PRINT CONCAT(N'Granted ', @newCode, N' to roles that had ', @legacyView, N': ', @@ROWCOUNT);
